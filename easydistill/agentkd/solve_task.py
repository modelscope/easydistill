...
# ==============================================================================
"""Entry point for solving synthesized virtual tool-use tasks.

Reads virtual tasks from a JSONL file (output of data_gen.py), expands each
task's test-case variants, and solves them using a mock-tool/mock-user LangGraph
agent. Solutions are written as JSON files per task.
"""
...
from infer_utils.graph.solve_task import run_solve_agent
...
def process_single_task(task_data: Dict[str, Any], config: Dict[str, Any]) -> bool:
    try:
        ...
        run_solve_agent(task_data, run_config=config)
        return True
    except Exception as e:
        tb = e.__traceback__
            ...
            try:
                ...
                for idx, task_and_background in enumerate(task_data["tasks_and_backgrounds"]):
                    ...
                    if new_task['id'] in processed_ids:
                        logger.info(f"Skipping processed task: {new_task['id']}")
                        continue
...
def main():
    parser = argparse.ArgumentParser(description='Solve synthesized virtual tool-use tasks')
