...
"""Single-file TI2I multi-model (multi-teacher) image-editing evaluation with Agentic Debate.

Self-contained single-file implementation of the TI2I evaluation line, following the
...
independent files ``t2i_multi_model.py`` / ``t2i_single_model.py`` /
``ti2i_single_model.py``. Functionality is preserved end to end:

1. TI2I 38-dim frozen pool (``ti2i_dimensions.json``), L1 -> L3,
   0-4 scale.
2. Multi-teacher scoring at case x teacher x L1-group granularity through
   ``ModelBackend`` (PAI Token / PAI EAS / OpenAI-compatible), judging the
   edit result against before/after/reference images.
...
5. Weighted-majority merge, plus optional reason synthesis on a separate
   ...
   per-case ``overall_score_100`` using the TI2I simple baseline aligned
   with T2I: 0/25/50/75/100 map, equal-weight mean of applicable scored L3
   dims, no family/L1/item weights, no gate capping.
...
Typical use::
    ...
    from easydistill.eval.ti2i_multi_model import TI2IMultiModelEvaluator
...
CLI (same seed jsonl format as the full pipeline)::
    ...
    python -m easydistill.eval.ti2i_multi_model \
        --config configs/eval/t2i_ti2i/ti2i_multi_model_pai_token.yaml
"""
...
DEFAULT_DIMENSIONS_FILE = _REPO_ROOT / "configs" / "eval" / "t2i_ti2i" / "ti2i_dimensions.json"
DEFAULT_PROMPTS_FILE = _REPO_ROOT / "configs" / "prompts" / "ti2i_multi_model_prompts.yaml"
...
class TI2IMultiModelEvaluator:
    """Multi-teacher TI2I evaluator with three-step Debate arbitration.
    ...
    Args:
        ...
        config:
            ...
            - prompts_file / prompts: prompt template overrides following the
              ...
              ``configs/prompts/ti2i_multi_model_prompts.yaml``).
    ...
    name = "ti2i_multi_model_evaluator"
    line = "ti2i"
    ...
    def _extract_case(self, sample: Dict[str, Any]) -> Tuple[str, str, List[str]]:
        """Return (case_id, edit instruction, image list) for one TI2I seed row."""
        case_id = str(sample.get("case_id") or sample.get("id") or "")
        instruction = str(sample.get("instruction") or sample.get("raw_prompt") or "")
        images = [img for img in (sample.get("before_image"), sample.get("after_image")) if img]
        images += list(sample.get("reference_images") or [])
        return case_id, instruction, [str(img) for img in images]

    def _case_context(self, instruction: str) -> str:
        return f"编辑指令：{instruction}（第一张为编辑前图，第二张为编辑后图）"
    ...
    # -- overall score (TI2I simple baseline aligned with T2I) ---------------
    ...
    def _compute_overall(self, judgments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "aggregation_method": "equal_weight_mean_t2i_score_map",
            "score_map_5": {str(k): v for k, v in SCORE_MAP_5.items()},
            "l1_subscores_100": {
            ...
            "note_zh": (
                "TI2I 简单 baseline：与 T2I 分制对齐，applicable L3 等权平均，"
                "无 family/L1/item 权重，无 gate 封顶。"
            ),
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TI2I 多模型（多教师 Debate）单文件评估")
    parser.add_argument(
        ...
        for sample in samples:
            for key in ("before_image", "after_image"):
                ...
                if value and not str(value).startswith(("http://", "https://", "/")):
                    sample[key] = str((seed_path.parent / str(value)).resolve())
