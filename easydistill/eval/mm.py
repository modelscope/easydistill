...
"""LLM-as-judge evaluation for multi-modal datasets."""
...
class MMInstructionFollowingEvaluator(LLMJudgeEvaluator):
    """Evaluate multi-modal instruction-following data with an LLM judge."""

    name = "mm_instruct_evaluator"
...
class MMCoTEvaluator(LLMJudgeEvaluator):
    """Evaluate multi-modal chain-of-thought data with an LLM judge."""

    name = "mm_cot_evaluator"
    DEFAULT_PROMPTS_FILE = "configs/prompts/default_cot_eval_prompts.yaml"
    BOOL_METRICS = {"logical_correctness"}

    def _extract_sample(self, sample: Dict[str, Any]) -> Tuple[str, str, str]:
        sample_id = str(sample.get("id", sample.get("index", 0)))
        instruction = (
            sample.get("instruction") or sample.get("input") or sample.get("problem") or ""
        )
