...
"""Single-file TI2I single-model (single-teacher) image-editing evaluation.

Self-contained single-file implementation of the TI2I single_model entry, following
...
the independent files ``t2i_multi_model.py`` / ``t2i_single_model.py``
/ ``ti2i_multi_model.py``. Semantics match the original pipeline's
single-teacher mode:
...
4. Optional reason synthesis: normalize the teacher reason via a separate
   ...
   synthesis).
5. Per-case ``overall_score_100`` using the TI2I simple baseline aligned
...
Typical use::
    ...
    from easydistill.eval.ti2i_single_model import TI2ISingleModelEvaluator

    evaluator = TI2ISingleModelEvaluator(teacher="qwen3.7-plus", backend=backend)
    results = evaluator.run(samples)
    summary = evaluator.aggregate(results)

CLI (same seed jsonl format as the full pipeline)::

    python -m easydistill.eval.ti2i_single_model \
        --config configs/eval/t2i_ti2i/ti2i_single_model_pai_token.yaml \
...
DEFAULT_PROMPTS_FILE = _REPO_ROOT / "configs" / "prompts" / "ti2i_single_model_prompts.yaml"
...
class TI2ISingleModelEvaluator:
    """Single-teacher TI2I evaluator (no cross-teacher conflict, no Debate).
    ...
    Args:
        ...
        config:
            ...
            - prompts_file / prompts: prompt template overrides following the
              ...
              ``configs/prompts/ti2i_single_model_prompts.yaml``).
    ...
    name = "ti2i_single_model_evaluator"
    line = "ti2i"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TI2I 单模型（单教师）单文件评估")
    parser.add_argument(
