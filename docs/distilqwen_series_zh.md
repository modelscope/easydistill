...
DistilQwen 家族来自 EasyDistill 工具链的蒸馏语言模型，在减小模型规模的同时保留教师模型的大部分能力，适合资源受限场景部署。

## 自适应思考模型

DistilQwen-ThoughtX 与 DistilQwen-ThoughtY 可生成长度与难度接近最优的思维链轨迹。ThoughtX 基于 OmniThought 数据集并采用 RV/CD 课程混合训练；ThoughtY 以 Qwen3 为学生模型、DeepSeek-R1-0528 为教师模型训练。

| 模型 | AIME2024 | MATH500 | GPQA-D | LCB V2 | 平均 | HuggingFace | ModelScope |
|---|---|---|---|---|---|---|---|
...
DistilQwen2 与 DistilQwen2.5 是指令跟随模型。DistilQwen2 以 GPT-4 与 Qwen-max 为教师并采用 DPO 排序优化；DistilQwen2.5 结合黑盒 SFT 与从 Qwen2.5-72B-Instruct 蒸馏的白盒知识。

| 模型 | AlpacaEval 2.0 (LC) | MT-Bench | MT-Bench (single) | IFEval (loose) | IFEval (strict) | HuggingFace | ModelScope |
|---|---|---|---|---|---|---|---|
...
DistilQwen2.5-R1 以 DeepSeek-R1 为教师并经 CogPO 算法精修；DistilQwen2.5-DS3-0324 从 DeepSeek-V3-0324 迁移快思考推理能力。

| 模型 | AIME2024 | MATH-500 | GPQA Diamond | LiveCodeBench V2 | HuggingFace | ModelScope |
|---|---|---|---|---|---|---|
